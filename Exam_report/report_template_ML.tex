...
\begin{document}
	...
	In questa sezione vengono riportati i risultati ottenuti dal modello fine-tunato in tre scenari distinti, con l'obiettivo di valutare sia la capacità del modello di adattarsi alla specifica calligrafia su cui è stato addestrato, sia il suo grado di generalizzazione a scritture diverse.
	
	\subsection{Metrica utilizzata}
	
	Dato che il modello lavora riga-per-riga e produce sequenze di caratteri, la valutazione naturale sarebbe basata su indicatori come il \emph{Character Error Rate} (CER) o il \emph{Word Error Rate} (WER). In questo progetto, tuttavia, l'aspetto più rilevante per l'utente finale (l'operatore che utilizza le trascrizioni per la ricerca documentale e per l'adempimento normativo) è la correttezza delle singole parole.
	
	Per questo motivo viene considerata la seguente metrica intuitiva:
	
	\[
	\text{Accuratezza parole} = \frac{\text{\# parole riconosciute correttamente}}{\text{\# parole totali}}.
	\]
	...
	Nel primo scenario il modello è stato testato su pagine appartenenti allo stesso quaderno utilizzato per il training, quindi:
	...
	In questo caso il modello ottiene, su più campioni di circa 100 parole ciascuno, un'accuratezza compresa tra l'87\% e il 90\%. Gli errori non sono quasi mai stravolgimenti completi della parola, ma piuttosto sostituzioni di singole lettere graficamente simili (ad esempio \emph{``casa''} riconosciuta come \emph{``caso''}). 
	
	Questi risultati indicano che il fine-tuning ha effettivamente permesso al modello di ``imparare'' bene la calligrafia specifica dell'autore del quaderno.
	
	\subsection{Scenario 2: altra persona, stessa tipologia di fogli}
	
	Nel secondo scenario l'OCR è stato applicato a pagine manoscritte da una persona diversa, ma su fogli con caratteristiche simili (righe pre-stampate, impaginazione confrontabile con quella del quaderno usato in addestramento). 
	
	In questo contesto l'accuratezza a livello di parola scende a circa il 60\% (ossia circa 60 parole corrette su 100). L'output rimane leggibile e utile come base di partenza, ma la quantità di correzioni manuali necessarie aumenta sensibilmente.
	
	Questa diminuzione di prestazioni mette in luce un aspetto previsto: con un dataset di addestramento relativamente piccolo (circa 150 pagine) il modello tende a specializzarsi sulla calligrafia di riferimento e fatica a generalizzare a scritture diverse, pur mantenendo comunque una certa capacità di riconoscere la struttura delle parole.
	
	\subsection{Ruolo della qualità di scansione}
	
	Durante i test è emerso che una parte consistente delle difficoltà non è dovuta solo alla calligrafia, ma anche alla qualità della scansione. Pagine acquisite con inclinazione eccessiva, contrasto basso o sfocatura locale risultano notevolmente più difficili da riconoscere, indipendentemente dal contenuto testuale.
	
	Questo suggerisce che, oltre al miglioramento del modello, un elemento cruciale per aumentare l'accuratezza complessiva è il controllo del processo di acquisizione (scanner adeguato, impostazioni stabili, eventuale normalizzazione delle immagini prima del riconoscimento).
	...
	Dal punto di vista applicativo, questi risultati mostrano che il modello fine-tunato, pur non essendo ancora utilizzabile in modo completamente automatico, è già in grado di ridurre in misura significativa il carico di trascrizione manuale per la calligrafia su cui è stato addestrato. Inoltre, i test su una seconda persona confermano che, con un aumento del numero di pagine annotate e un controllo migliore della qualità di scansione, è realistico puntare a livelli di accuratezza più elevati anche per popolazioni di scriventi più ampie, come discusso nelle conclusioni.
